...
function response(data = { type: 'NEW_DRIVER_OFFER', requestId: 'job1' }, identifier = 'tap1') {
...
test('retains a cold-start tap until session and navigator are ready', async () => {
  ...
  mockNavigationState = { key: 'root' };
  await render();
  expect(mockPush).toHaveBeenCalledWith({ pathname: '/request-status', params: { requestId: 'job1' } });
