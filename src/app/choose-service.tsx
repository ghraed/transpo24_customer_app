...
export default function ChooseServiceScreen() {
  ...
  const loadServices = useCallback(async () => {
    ...
    try {
      ...
      setServices(list);
      if (list.length === 0) setSelectedService(null);
    } catch (err) {
  ...
  if (services.length === 0) {
    return (
      <View style={styles.centerContainer}>
        <Text style={styles.errorTitle}>No services available</Text>
        ...
        renderItem={({ item }) => {
          ...
          return (
            ...
            >
              ...
              <Text style={styles.cardDescription}>{item.descriptionEn}</Text>
            </Pressable>
          );
        }}
      />
...
const styles = StyleSheet.create({
  ...
  subtitle: { fontSize: 15, color: '#475467', marginTop: 4 },
  ...
  stateText: { marginTop: 10, color: '#475467', textAlign: 'center', fontSize: 14 },
  errorTitle: { fontSize: 20, fontWeight: '700', color: '#101828', marginBottom: 2 },
  retryButton: {
    marginTop: 16,
    backgroundColor: '#1a73e8',
    borderRadius: 10,
