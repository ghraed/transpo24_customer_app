...
import { clientTheme } from './tracking-ui';
import { pickChatAttachments, sendSelectedChatAttachments } from '@/lib/chat-files';
        ...
        <FlatList data={files} numColumns={2} keyExtractor={(file, index) => `${file.uri}-${index}`}
          ...
          renderItem={({ item, index }) => <View style={styles.card}>
            {item.mimeType?.startsWith('image/') || /\.(jpe?g|png)$/i.test(item.name)
              ...
              : <View style={styles.preview}><ChatIcon name="document" size={48} color="#9A6500" /></View>}
...
const styles = StyleSheet.create({
  ...
  send: { backgroundColor: clientTheme.accent, borderRadius: 24, margin: 16, minHeight: 48, alignItems: 'center', justifyContent: 'center', padding: 12 },
