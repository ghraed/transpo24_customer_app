...
import { openChatFile } from '@/lib/chat-files';
jest.mock('@/components/tracking-ui', () => ({ clientTheme: { accent: '#FFC548' } }));
jest.mock('react-i18next', () => ({ useTranslation: () => ({ t: key => key }) }));
jest.mock('@/lib/chat-files', () => ({ openChatFile: jest.fn(async () => undefined), downloadChatFile: jest.fn() }));
it('opens the full group from +N and makes files beyond the preview viewable', async () => {
  ...
  expect(openChatFile).toHaveBeenCalledWith('/request-files/5/content', '5.pdf');
