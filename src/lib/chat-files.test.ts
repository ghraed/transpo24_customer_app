...
it('never sends credentials to a URL supplied by another host', async () => {
  await expect(
    openChatFile('https://example.com/steal', 'file.pdf'),
  ).rejects.toThrow();
...
it('opens an authenticated attachment in a viewer and keeps it readable after launch', async () => {
  ...
  await openChatFile('/request-files/file/content', 'file.pdf');
...
it('does not open an unauthorized response and still cleans up', async () => {
  ...
  await expect(
    openChatFile('/request-files/file/content', 'file.pdf'),
  ).rejects.toThrow();
  expect(Sharing.shareAsync).not.toHaveBeenCalled();
...
it('cleans up a failed viewer launch without opening the share menu', async () => {
  ...
  await expect(openChatFile('/request-files/file/content', 'file.pdf')).rejects.toThrow('documents.openFailed');
  expect(Sharing.shareAsync).not.toHaveBeenCalled();
  expect(FileSystem.deleteAsync).toHaveBeenCalled();
});
it('saves an Android attachment to the folder the client chooses', async () => {
  Platform.OS = 'android';
  jest.mocked(FileSystem.downloadAsync).mockResolvedValue({ status: 200, uri: 'file:///cache/file.pdf', headers: {}, mimeType: 'application/pdf' });
  expect(await downloadChatFile('/request-files/file/content', 'file.pdf')).toBe(true);
...
it('does not save or report success when the folder picker is cancelled', async () => {
  ...
  expect(await downloadChatFile('/request-files/file/content', 'file.pdf')).toBe(false);
...
it('offers the iOS export sheet only for an explicit download', async () => {
  Platform.OS = 'ios';
  await downloadChatFile('/request-files/file/content', 'file.pdf');
...
it('rejects untrusted download URLs before contacting them', async () => {
  await expect(downloadChatFile('https://example.com/file', 'file.pdf')).rejects.toThrow();
  expect(FileSystem.downloadAsync).not.toHaveBeenCalled();
});
...
it('removes a partial saved file when writing fails', async () => {
  ...
  await expect(downloadChatFile('/request-files/file/content', 'file.pdf')).rejects.toThrow('Disk full');
