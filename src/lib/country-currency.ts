...
const EURO_COUNTRY_CODES = new Set<CountryCode>([
  'AD',
  'AT',
  'AX',
  'BE',
...
export function normalizeCountryCode(countryCode?: string | null): CountryCode | null {
  const normalized = countryCode?.trim().toUpperCase() ?? '';
  return normalized.length === 2 ? (normalized as CountryCode) : null;
}
